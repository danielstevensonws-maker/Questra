...
describe('the table controls belong to whoever runs the game', () => {
  it('refuses to let a player start the fight', () => {
    const out = makeSliceResolver()(envelope({ kind: 'start_combat' }), state(), PLAYER, { playSessionId: 'ps_test', log: [] });
  ...
  it('refuses to let a player skip a turn', () => {
    ...
    expect(makeSliceResolver()(envelope({ kind: 'advance_turn' }), s, PLAYER, { playSessionId: 'ps_test', log: [] }).ok).toBe(false);
  });

  it('lets the DM start the fight, and opens on somebody', () => {
    const out = makeSliceResolver()(envelope({ kind: 'start_combat' }), state(), DM, { playSessionId: 'ps_test', log: [] });
  ...
  it('will not start a fight in an empty room', () => {
    const out = makeSliceResolver()(envelope({ kind: 'start_combat' }), state({ combatants: {} }), DM, { playSessionId: 'ps_test', log: [] });
  ...
  it('ends a fight by clearing the order, so exploring is one state not two', () => {
    const s = state({ order: ['mira', 'goblin'], activeCreatureId: 'mira' });
    const out = makeSliceResolver()(envelope({ kind: 'end_combat' }), s, DM, { playSessionId: 'ps_test', log: [] });
    expect(out.ok).toBe(true);
    if (!out.ok) return;
  ...
  it('will not end a fight nobody is in', () => {
    expect(makeSliceResolver()(envelope({ kind: 'end_combat' }), state(), DM, { playSessionId: 'ps_test', log: [] }).ok).toBe(false);
  });
});
...
describe('taking your turn', () => {
  ...
  it('allows it while exploring, because that is how a fight starts', () => {
    const out = makeSliceResolver()(
      envelope({ kind: 'attack', attackerId: 'mira', targetId: 'goblin', actionName: 'Longsword' }),
      state(), PLAYER, { playSessionId: 'ps_test', log: [] });
    expect(out.ok).toBe(true);
  });
});
...
describe('whispering', () => {
  it('addresses the whisper through visibility, where the one filter reads it', () => {
    const out = makeSliceResolver()(
      ...
      state(), DM, { playSessionId: 'ps_test', log: [] });
    expect(out.ok).toBe(true);
    if (!out.ok) return;
